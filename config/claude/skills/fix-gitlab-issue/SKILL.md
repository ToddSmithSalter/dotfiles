...
description: Fix Gitlab issues using glab CLI. Use when asked to fix, resolve, or address a Gitlab issue. Creates fixes on separate branches, runs tests locally, and creates MRs when tests pass. Requires glab CLI authenticated and repo cloned locally.
---

# Gitlab Issue Fixer

Fix Gitlab issues by implementing solutions on feature branches with proper testing.
...
gh pr create --title "Fix #<issue-number>: <title>" --description "Fixes #<issue-number>
...
If CI fails:
1. Review the failure: `glab ci view --pipelineid <pipeline-id>`
2. Fix locally
